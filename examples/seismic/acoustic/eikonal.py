...
class EikonalSolver:
    ...
    def _validate_model_coverage(self):
        ...
        min_survey_x = min(
            self.seismogram_dataset.x_coords.min(),
            self.seismogram_dataset.opposite_x.min(),
        )
        max_survey_x = max(
            self.seismogram_dataset.x_coords.max(),
            self.seismogram_dataset.opposite_x.max(),
        )
        min_survey_z = min(
            self.seismogram_dataset.elevations.min(),
            self.seismogram_dataset.opposite_elev.min(),
        )
        max_survey_z = max(
            self.seismogram_dataset.elevations.max(),
        ...
        # Check coverage
        if min_survey_x < model_x[0] or max_survey_x > model_x[-1] or min_survey_z < model_z[0] or max_survey_z > model_z[-1]:
            raise ValueError("Velocity model does not cover the entire survey area. " "Consider padding the model.")
    ...
    def _initialize_solver(self) -> PointSourceSolver:
        ...
        solver.velocity.min_coords = (
            self.velocity_model.z[0],
            self.velocity_model.x[0],
            0,
        )
        solver.velocity.node_intervals = (
            self.velocity_model.dz,
            self.velocity_model.dx,
            1,
        )
