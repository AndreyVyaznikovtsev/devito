...
def compute_gradient_batch(shot_ids, fk_down, fk_up, dt):
    ...
    # grad_up = calc_grad_batch(u0_up.numpy(), v_down.numpy(), dt=dt)
    # grad_down = calc_grad_batch(u0_down.numpy(), v_up.numpy(), dt=dt)
    
    # # Combine gradients and compute illumination
    # grad_batch = grad_up + grad_down
    
    grad_batch = calc_grad_batch(u0_batch, v_batch, dt=dt)
    return grad_batch / (torch.sum(u0_batch**2, dim=1) + 1e-9)
    # return grad_batch
...
def main_compute_gradients_batched(iter, batch_size=4):
    ...
    grad_full = torch.zeros((sub_nx, sub_nz), device='cuda')
    ...
    # Save results
    np.save(f"{OUTPUT_DIRS['gradients']}/grad_full_{iter}.npy", grad_full.cpu().numpy())
