...
def compute_gradient_batch(shot_ids, fk_down, fk_up, dt, scalers=None):
    ...
    if scalers is None:
        return (grad/1 for grad in [grad_up, grad_down])
    else:
...
def main_compute_gradients_batched(iter, batch_size=4):
    ...
    for batch_start in range(0, num_shots, batch_size):
        ...
        # scalers = None
        grad_u_batch, grad_d_batch = compute_gradient_batch(shot_ids, fk_down=fk_down, fk_up=fk_up, dt=TMAX/(NSNAPS+1), scalers=None)
