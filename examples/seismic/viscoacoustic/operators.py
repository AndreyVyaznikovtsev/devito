...
def sls_2nd_order(model, geometry, p, r=None, **kwargs):
    ...
    if forward:
        ...
        pde_p = (
            m * p.dt2
            - div(b * grad((1.0 + tt) * rho * p, shift=0.5), shift=-0.5)
            - div(b * grad(rho * r.backward, shift=0.5), shift=-0.5)
            + (1 - damp) * p.dt.T
        )
        u_p = Eq(p.backward, damp * solve(pde_p, p.backward))
...
def kv_2nd_order(model, geometry, p, **kwargs):
    ...
    if forward:
        ...
        pde_p = (
            m * p.dt2
            - rho * div(b * grad(p, shift=0.5), shift=-0.5)
            - tau * rho * div(b * grad(p.dt(x0=t0), shift=0.5), shift=-0.5)
            + (1 - damp) * p.dt
        )
        ...
        pde_p = (
            m * p.dt2
            - div(b * grad(rho * p, shift=0.5), shift=-0.5)
            - div(b * grad(rho * tau * p.dt(x0=t0).T, shift=0.5), shift=-0.5)
            + (1 - damp) * p.dt.T
        )
...
def ForwardOperator(model, geometry, space_order=4, kernel="sls", time_order=2, save=False, **kwargs):
    ...
    return Operator(
        eqn + src_term + rec_term,
        subs=model.spacing_map,
        name="ViscoIsoAcousticForward",
        **kwargs,
    )
