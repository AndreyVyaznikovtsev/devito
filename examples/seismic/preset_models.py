...
def demo_model(preset, **kwargs):
    ...
    * 'marmousi3d-tti': Loads the 2D Marmousi data set from the given
    ...
    if preset.lower() in ["constant-viscoacoustic"]:
        ...
        # velocity
        vp = 1e-3 * np.fromfile(
            os.path.join(data_path, "marmousi3D/MarmousiVP.raw"),
            dtype="float32",
            sep="",
        )
        vp = vp.reshape(shape)
        ...
        epsilon = (
            np.fromfile(
                os.path.join(data_path, "marmousi3D/MarmousiEps.raw"),
                dtype="float32",
                sep="",
            )
            * 1e-2
        ...
        delta = (
            np.fromfile(
                os.path.join(data_path, "marmousi3D/MarmousiDelta.raw"),
                dtype="float32",
                sep="",
            )
            * 1e-2
        ...
        theta = np.fromfile(
            os.path.join(data_path, "marmousi3D/MarmousiTilt.raw"),
            dtype="float32",
            sep="",
        )
        ...
        if preset.lower() in ["marmousi-tti3d", "marmousi3d-tti"]:
            ...
            phi = np.fromfile(
                os.path.join(data_path, "marmousi3D/Azimuth.raw"),
                dtype="float32",
                sep="",
            )
