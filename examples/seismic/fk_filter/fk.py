...
class FKFilter3D:
    ...
    def _compute_filter(self, Z: int, X: int, T: int):
        ...
        fzz = self.freq_z.view(-1, 1, 1)  # (Z, 1, 1)
        fxx = self.freq_x.view(1, -1, 1)   # (1, X, 1)
        ftt = self.freq_t.view(1, 1, -1)   # (1, 1, T//2+1)
        
        # Calculate angular frequencies
        omega = 2 * np.pi * ftt  # (1, 1, T//2+1)
        kx = 2 * np.pi * fxx     # (1, X, 1)
        kz = 2 * np.pi * fzz     # (Z, 1, 1)
        ...
        with torch.no_grad():
            ...
            # Create mask
            mask = (v_apparent > (self.min_slope + self.lower_min)) & \
                   (v_apparent < (self.max_slope + self.upper_min))
            ...
            if self.sigma_z > 0 or self.sigma_x > 0:
                buff = self._gaussian_filter_2d(buff)
            
            # Frequency attenuation
            k_sq = (kx**2 + kz**2)  # (Z, X, 1)
            attenuation = 1 - torch.exp(-k_sq / (2 * (2*np.pi*self.gaussian_sigma)**2))
            self.filter = buff * attenuation
    ...
    def _gaussian_filter_2d(self, x: torch.Tensor) -> torch.Tensor:
        ...
        if kernel_z is not None:
            ...
            x_zt = torch.nn.functional.conv2d(
                x_zt,
                weight=kernel_z.view(1, 1, -1, 1),
                padding=(padding_z, 0)
            )
        ...
        if kernel_x is not None:
            ...
            x_xt = torch.nn.functional.conv2d(
                x_xt,
                weight=kernel_x.view(1, 1, 1, -1),
                padding=(0, padding_x)
            )
    ...
    def __call__(self, input: np.ndarray):
        ...
        print(spectrum.numel() * spectrum.element_size()/1024/1024)
    ...
    def plot_filter_slice(self, t_idx=0, lims=None):
        ...
        plt.imshow(slice,
                 extent=[freq_x.min(), freq_x.max(),
                         freq_z.min(), freq_z.max()],
