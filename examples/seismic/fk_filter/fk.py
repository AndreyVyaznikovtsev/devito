...
class FKFilter3D:
    ...
    def _compute_filter(self, Z: int, X: int, T: int):
        ...
        with torch.no_grad():
            ...
            mask = (fzz > lower_bound) if self.min_slope > 0 else (fzz < lower_bound)
            if self.max_slope > 0:
                mask = ~mask
            ...
            t_mask_high = 1 / (1 + (torch.abs(self.freq_t) / self.high_cut) ** self.high_slope)
            if self.low_cut > 0:
                t_mask_low = 1 - 1 / (1 + (torch.abs(self.freq_t) / self.low_cut) ** self.low_slope)
            else:
                t_mask_low = 1.
            t_mask = (t_mask_high * t_mask_low).view(1, 1, -1)

            filter_3d = filter_3d * t_mask # * (1-t_mask2)
    ...
    def plot_filter_slice(self, t_idx=0, lims=None):
        ...
        plt.xlabel(r"$k_x$ (Рад/м)")
        plt.ylabel(r"$k_z$ (Рад/м)")
        if self.min_slope > 0:
            plt.title(f"$k_x-k_z$ срез F-K фильтра для $P_u(t,x,z)$")
        else:
            plt.title(f"$k_x-k_z$ срез F-K фильтра для $P_d(t,x,z)$")
            
        divider = make_axes_locatable(plt.gca())
        cax = divider.append_axes("right", size="5%", pad=0.05)
        cbar = plt.colorbar(im, cax=cax, label="A, у.е.")
        
        # Get current tick positions
        # ticks = cbar.get_ticks()
        # ticks = [ticks[0], ticks[-1]]
        # # Set custom tick labels: only +, 0, and -
        # tick_labels = ['0', '1']
