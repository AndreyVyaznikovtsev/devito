...
def taper_traces_vectorized(data, first_breaks, time_axis, window_before=0, window_after=0.1):
    ...
    tapers = np.array([
        create_taper_window(nt, time_axis, fb_times[i], window_before, window_after)
        for i in range(ntr)
    ]).T
...
def wiener_deconvolution(obs, modeled, sz, rec_z, time_axis, first_breaks=None,
                        eps=1e-6, normalize=True, kill_offset=False, offset_threshold=0,
                        taper_before=0, taper_after=0.1):
    """
    ...
    sumn, sumd, used_traces = compute_fft_components(
        obs, modeled, offsets, 
        offset_threshold if kill_offset else -np.inf
    )
