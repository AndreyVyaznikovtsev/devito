...
def compute_wavefields(model, dataset, shot_id):
    """Compute and save forward wavefield snaps"""
    objective = 0.0
    t0 = 0.0
    tn = TMAX
...
def main():
    ...
    model, dataset, _ = setup_truemodel_and_geometry(PATH_DATA_NOGAL)
    dataset._dt_r = model.critical_dt
    dataset._t_max_r = TMAX
    dataset.resample_on()
    print("Num samples :", dataset._t_max_r/model.critical_dt)
