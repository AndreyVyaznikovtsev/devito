...
def load_current_dm(iter_num):
    if iter_num == 0:
        ...
        dm = np.pad(dm, ((NBL, NBL), (0, NBL)), mode='constant', constant_values=0.)
    return dm
...
def compute_forward_snaps(model, dataset, shot_id):
    ...
    wav_data_source = np.load(f"{PATH_WAVELETS}/wavelet_{shot_id}_norm_alt.npy")
    ...
    np.save(f"{OUTPUT_DIRS['forward_snaps']}/{shot_id+1}.npy", u0.data[:, NBL//SUBSAMPLING:-NBL//SUBSAMPLING, :-NBL//SUBSAMPLING])
    np.save(
        f"{OUTPUT_DIRS['forward_snaps']}/recon_gather_{shot_id+1}.npy",
        d_syn.data[:],
    )
...
def compute_wavefields(model, dataset_dplus, dataset, shot_id, iter_num):
    """Compute and save forward wavefield snaps"""
    objective = 0.0
    t0 = 0.0
    tn = TMAX
    ...
    np.save(f"{OUTPUT_DIRS['adjoint_snaps']}/{shot_id+1}.npy", v.data[:, NBL//SUBSAMPLING:-NBL//SUBSAMPLING, :-NBL//SUBSAMPLING])
...
def main():
    ...
    model, dataset_dplus, dataset, _ = setup_model_and_geometry(PATH_DATA_DSUB)
    dataset._dt_r = model.critical_dt
    dataset._t_max_r = TMAX
    dataset.resample_on()

    dataset_dplus._dt_r = model.critical_dt
    dataset_dplus._t_max_r = TMAX 
    ...
    # for i in SHOT_IDS:
        ...
        obj_shot = compute_wavefields(model, dataset_dplus, dataset, i,
                                      iter_num=args.iter
                                      )
        objective += obj_shot
        if i % 5 == 0:
